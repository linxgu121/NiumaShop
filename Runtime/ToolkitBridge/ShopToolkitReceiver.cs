...
namespace NiumaShop.Bridge
{
    /// <summary>
    /// 商店 UI Toolkit 接收器。
    /// 挂在 UIRoot/UIBridges 下，并拖给 ShopUIViewBridge 的 Shop UI Receiver Provider。
    /// </summary>
    public sealed class ShopToolkitReceiver : MonoBehaviour, IShopUIReceiver
    {
        ...
        [Tooltip("收到 Cleared 更新时是否关闭商店窗口。")]
        ...
        {
            if (update.UpdateType == ShopUIUpdateType.Cleared)
            {
        ...
        private void Warn(string message)
        {
            if (logWarnings)
            {
                Debug.LogWarning($"[ShopToolkitReceiver] {message}", this);
            }
        }
